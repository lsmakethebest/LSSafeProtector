...
# 注意
-  最近好多人问我为什么线上环境崩溃位置定位不到，这是因为导出ipa包安装，崩溃位置是定位不到的，即使Debug模式导出ipa也是定位不到，和正式测试没关系，是由于ipa包安装的crash日志是非源码，无法直接分析定位，必须符号化。具体符号化步骤可以参照网上，这里不做过多说明，本框架的主旨是防止crash，而不是定位crash
